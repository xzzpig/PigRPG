...
public class Equipment extends ItemStack {
	...
	public Equipment setEquipQuality(EquipQuality equality) {
		...
		this.setDisplayName(
				this.getItemMeta().getDisplayName()
						.replaceFirst(this.equality.toString(), ""), equality);
	...
	public String getLoreData(String key) {
		return new TArgsSolver(getItemMeta().getLore().toArray(new String[0]))
				.get(key);
	}
		...
		for (String lore : lores)
			for (PowerLore pl : PowerLore.powerlores) {
				if (lore.contains(pl.getKey()))
					powerlores.add(pl.setEquip(this).clone().loadVars(lore)
							.loadPowers());
			}
