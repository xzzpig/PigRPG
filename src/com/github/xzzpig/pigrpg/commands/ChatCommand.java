...
import com.github.xzzpig.pigrpg.Voids;
...
public class ChatCommand {
	public static boolean command(CommandSender sender, Command cmd,
		...
		if (!User.getUser((Player) sender).hasPremission(
				Premissions.pigrpg_command_chat_default)) {
		...
		if (getarg(args, 1).equalsIgnoreCase("help")) {
			...
			((Player) sender).openInventory(ChatChannelChest
					.getAcceptInventory(User.getUser((Player) sender)));
			return true;
		} else if (getarg(args, 1).equalsIgnoreCase("change")) {
			((Player) sender).openInventory(ChatChannelChest
					.getChooseInventory(User.getUser((Player) sender)));
			return true;
			...
			sender.sendMessage(TString.Prefix("PigRPG", 3) + "已改变玩家"
					+ user.getPlayer().getName() + "禁言的状态");
			return true;
			...
			if (sboolean.equalsIgnoreCase(""))
				...
				Vars.nms.newFancyMessage(TString.Prefix("PigRPG", 4) + "请设置")
						...
						.then(ChatColor.RED + " 或 ")
						.then(ChatColor.GREEN.toString() + ChatColor.UNDERLINE
								+ "false").tooltip("/pr chat muteall false")
